...
class YamlConfigurationContext implements Context
{
    ...
    public function addTemplateToImortCode($templateName, $code)
    {

        $destinationFilePath = 'app/Resources/views/'.$templateName;
        if (!file_exists($destinationFilePath))
        {
            file_put_contents($destinationFilePath, $code);
        }
    }
